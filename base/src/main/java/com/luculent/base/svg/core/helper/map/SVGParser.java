package com.luculent.base.svg.core.helper.map;
...
public class SVGParser {
    ...
    private static Matrix parseTransformItem(String s, Matrix matrix) {
        if (s.startsWith("matrix(")) {
            ...
            if (np.numbers.size() == 6) {
                ...
                double tag0 = np.numbers.get(0);
                double tag1 = np.numbers.get(1);
                double tag2 = np.numbers.get(2);
                double tag3 = np.numbers.get(3);
                double tag4 = np.numbers.get(4);
                double tag5 = np.numbers.get(5);

                mat.setValues(new float[]{
                        // Row 1
                        (float) tag0, (float) tag2, (float) tag4,
                        // Row 2
                        (float) tag1, (float) tag3, (float) tag5,
                        // Row 3
                        0, 0, 1,});
    ...
    private static Path doPath(String s) {
        ...
        while (ph.pos < n) {
            ...
            switch (cmd) {
                case 'M':
                case 'm': {
                    double x = ph.nextFloat();
                    double y = ph.nextFloat();
                    if (cmd == 'm') {
                        subPathStartX += x;
                        subPathStartY += y;
                        p.rMoveTo((float) x, (float) y);
                        lastX += x;
                        lastY += y;
                    } else {
                        subPathStartX = x;
                ...
                case 'z': {
                    p.close();
                    p.moveTo((float) subPathStartX, (float) subPathStartY);
                    lastX = subPathStartX;
                    lastY = subPathStartY;
                ...
                case 'c': {
                    wasCurve = true;
                    double x1 = ph.nextFloat();
                    double y1 = ph.nextFloat();
                    double x2 = ph.nextFloat();
                    double y2 = ph.nextFloat();
                    double x = ph.nextFloat();
                    double y = ph.nextFloat();
                    if (cmd == 'c') {
                        x1 += lastX;
                    ...
                    p.cubicTo((float) x1, (float) y1, (float) x2, (float) y2, (float) x, (float) y);
                    lastX1 = x2;
                    lastY1 = y2;
                    lastX = x;
                    lastY = y;
                ...
                case 's': {
                    ...
                    double x1 = 2 * lastX - lastX1;
                    double y1 = 2 * lastY - lastY1;
                    p.cubicTo((float) x1, (float) y1, (float) x2, (float) y2, (float) x, (float) y);
                    lastX1 = x2;
                    lastY1 = y2;
                    lastX = x;
                ...
                case 'A':
                case 'a': {
                    double rx = ph.nextFloat();
                    double ry = ph.nextFloat();
                    double theta = ph.nextFloat();
    ...
    private static void drawArc(Path p, double lastX, double lastY, double x, double y, double rx, double ry, double theta, int largeArc, int sweepArc) {
        ...
        if ((theta % 360) == 0) {
            ...
            arcRectf.set((float) -rx, (float) -ry, (float) rx, (float) ry);

            arcMatrix.reset();
            arcMatrix.postRotate((float) theta);
    ...
    private static double getFloatAttr(String name, Attributes attributes, double defaultValue) {
        String v = getStringAttr(name, attributes);
        return parseFloatValue(v, (float) defaultValue);
    }
    ...
    private static class LayerAttributes {
        ...
        public LayerAttributes(double opacity) {
    ...
    static class SVGHandler extends DefaultHandler {
        ...
        private Gradient doGradient(boolean isLinear, Attributes atts) {
            ...
            if (isLinear) {
                gradient.x1 = (float) getFloatAttr("x1", atts, 0f);
                gradient.x2 = (float) getFloatAttr("x2", atts, 1f);
                gradient.y1 = (float) getFloatAttr("y1", atts, 0f);
                gradient.y2 = (float) getFloatAttr("y2", atts, 0f);
            } else {
        ...
        private void doColor(Properties atts, Integer color, boolean fillMode, Paint paint) {
            ...
            double opacity = opacityAttr != null ? opacityAttr : 1f;
        ...
        private void pathStyleHelper(String style, String offset) {
            ...
            while (st.hasMoreTokens()) {
                intervals[i++] = (float) (current = toFloat(st.nextToken(), current));
                max += current;
            }
            ...
            strokePaint.setPathEffect(new DashPathEffect(intervals, (float) off));
        }

        private static double toFloat(String s, double dflt) {
            double result = dflt;
            try {
        ...
        private void doLimits2(double x, double y) {
            if (x < limits.left) {
                limits.left = (float) x;
            }
            if (x > limits.right) {
                limits.right = (float) x;
            }
            if (y < limits.top) {
                limits.top = (float) y;
            }
            if (y > limits.bottom) {
                limits.bottom = (float) y;
            }
        }
        ...
        private void doLimits(RectF box, Paint paint) {
            ...
            double width2 = (paint == null) ? 0 : paint.getStrokeWidth() / 2;
            ...
            if (localName.equals("svg")) {
                ...
                if (gradient != null) {
                    ...
                    if (stopColour == null) {
                        ...
                        double alpha = props.getFloat("stop-opacity", 1) * currentLayerAttributes().opacity;
                        int alphaInt = (int) Math.round(255 * alpha);
                        colour = stopColour.intValue() | (alphaInt << 24);
                    }
                    gradient.colors.add(colour);

                    double offset = props.getFloat("offset", 0);
                    gradient.positions.add((float) offset);
                }
                ...
                final double opacity = props.getFloat("opacity", 1f);
                ...
                Float width = getFloatAttr("width", atts);
                Float height = getFloatAttr("height", atts);
                double rx = getFloatAttr("rx", atts, 0);
                double ry = getFloatAttr("ry", atts, 0);
                pushTransform(atts);
                ...
                if (doFill(props, rect)) {
                    ...
                    if (rx <= 0f && ry <= 0f) {
                        ...
                        canvas.drawRoundRect(rect, (float) rx, (float) ry, fillPaint);
                    }
                    doLimits(rect);
                }
                if (doStroke(props)) {
                    ...
                    if (rx <= 0f && ry <= 0f) {
                        ...
                        canvas.drawRoundRect(rect, (float) rx, (float) ry, strokePaint);
                    }
                ...
                if (numbers != null) {
                    ...
                    if (points.size() > 1) {
                        pushTransform(atts);
                        Properties props = new Properties(atts);
                        double point0 = points.get(0);
                        double point1 = points.get(1);
                        p.moveTo((float) point0, (float) point1);
...
//
                        ...
                        gradient.shader = new RadialGradient((float) gradient.x, (float) gradient.y, (float) gradient.radius, colors, positions, gradient.tilemode);
                    }
